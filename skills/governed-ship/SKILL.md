...
5. Record pr_url and transition plan to merged only after the host confirms merge; then mark epic merged in a follow-up metadata commit. A new implementation commit invalidates both final reviews. Return plan to in-progress through the validator and repeat code review before AppSec review after fixes. Metadata-only approval commits do not invalidate final reviews.
