...
| Staff code reviewer (final plan review) | code-reviewer, code-review-and-quality, /review | requesting-code-review, receiving-code-review | agent-skills owns five-axis rubric after all tasks and full QA; Superpowers dispatches and handles feedback |
...
Do not run both TDD skills. agent-skills' test-driven-development is deliberately excluded from the installer. Incremental implementation must defer to Superpowers' task execution. Tasks need local RED/GREEN/QA evidence and separate commits; staff code-review approval belongs to the final plan review, with a new review after later implementation changes. Local governed-plan/build/ship wrappers are mandatory outer workflows, including when a lifecycle slash command is invoked directly. Upstream fast paths, direct local merge, and work-in-place fallback cannot bypass project gates or epic isolation.
